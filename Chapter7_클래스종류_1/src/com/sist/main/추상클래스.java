 ...
 *   							class B extends A
 *   							=> A a=new B();
 *   							=> A a=new A(); => 오류 자신으로 메모리 할당 못함
 *   					2) 메소드명 동일
 *   					3) 리턴형 동일
 *   					4) 매개변수가 동일 (매개변수가 다른 경우 => 오버로딩)
 *   					5) 접근지정어는 확대가능 / 축소하면 오류 발생
 *   					   -------
 ...
 *   				= 프로그램에 맞게 구현해서 사용
 *   				  예)
 *   					 void getConnection(); 구현이 안되어 있어야함
 *   					 => 데이터베이스 연결
 ...
 *   					댓글형게시판 : : 목록출력, 찾기, 글쓰기, 내용보기, 수정하기, 삭제하기, 댓글
 *   					후기 게시판 목록 출력: : 목록출력, 찾기, 글쓰기, 내용보기, 수정하기, 삭제하기, 업로드
 *   					---
 *   					---
 *   					---
 *   
 ...
 *   					 ->---
 *   					
 *   				abstract class Board 공통으로 쓰는것은 구현해서 아닌것은 선언만해서
 *   				{
 *   					목록출력();
 *   					찾기(){} 
 *   					글쓰기();
 ...
 *   									    ------ 구현이 안된 메소드
 *   					abstract void find(); //구현이 안된 메소드 선언만 되어있는 추상메소드 우리가 구현 해야함
 *   					Board content() {}
 *   					void write() {}
 *   
 *   	추상클래스
 ...
 *   		abstract class A
 *   		class B extends A
 *   		----------------- 구현이 안된 메소드를 구현한다 상속받아서 구현
 *   		=> A a=new B(); 묵시적 형변환
 *   			double d=10; 자동 형변환
 *   		=> A a=new B();
 *   			B b=(B)a; ====> 명시적 형변환
 *   		*** 클래스는 상속관계, 포함관계 => 크기를 잴수 있다
 *   		상속을 내리는 클래스 > 상속을 받는 클래스 보다 크다
 ...
 *  1. 일반 클래스 => new (O)
 *  2. 추상 클래스 => 구현후 사용
 *  3. 인터 페이스 => 추상클래스와 동일 (O)
 *  4. 종단 클래스(final 클래스) => 상속해서 황장이 불가능 상속못함
 *  5. static 클래스 => 메모리 => 클래스명으로 접근
 *  --------------------------------------
 */
abstract class A
{
	public abstract void aaa(); // 구현해야됨
	public  void bbb() {}
	public  void ccc() {}
	public  void ddd() {}
	public abstract void eee(); // 구현해야됨
}
class B extends A
{
