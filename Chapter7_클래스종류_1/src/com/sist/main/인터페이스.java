 ...
 *  -------------------------------------------
 *  |             |           |		 |		 |
 *  ArrayList   LinkedListe	Vector Queue   Stack
 *  
 ...
 *  	2) 메소드 ==> void aaa()
 *  				{
 *  					=> return; //코딩하지 않으면 리턴 자동 추가
 *  				}
 *  	3) 생성자 => class A
 *  				{
 *  					=>A(){}
 *  				}
 *  	4) 인터페이스 모든변수 메소드 public
 ...
 *      interface A
 *      {
 *        (public abstract)void aaa();// 퍼블릭 생략
 *      }
 *      class B implements A
 *      {
 *      	void aaa(){} =>오류
 ...
 *       class B implements A
 *      {
 *      	public void aaa(){} //접근지정어 퍼블릭 줘야됨
 *      }
 *      
 *      249page 
 *      1) 인터페이스는 객체를 생성할 수 없다 (구현이 안된 메소드를 가지고 있기 떄문에
 *      	메모리에 저장이 불가능)
 *      	인터페이스 =new 구현된 클래스() // 구현을 하고 구현된 것을 갖다놔야 생성
 *      	==> (익명의 클래스를 이용하면 객체를 생성할 수 있다) 
 *      2) 인터페이스는 다중상속이 가능하다
...
//{
	//public int a=10;
	//public void display(); //(public abstract) 자동추가
//}
public class 인터페이스 {
