 ...
 * 4. 생성자를 사용하지 않은면 자동으로 기본 생성자가 첨부...
 * 5. 사용처
 ...
 *  new A()
 *  
 *  *****인스턴스 메소드 : 인스턴스변수, 메소드,static변수 static메소드
 *  					전체를 사용이 가능
 *  ***** static 메소드 : static변수 , static메소드만 사용이 가능
 ...
 * 		 -------------------------------
 * 		 void aaa() =================> aaa()
 * 		int aaa(int a) ==============> aaa(int)----| 얘네 두개는 같은거 오버로딩 아님
 * 		double aaa(int a) ===========> aaa(int)----| 같은 메소드로 인식
...
class Student{
	...
	void display()
	{
		...
		System.out.println(this.name); //this. 클래스 꺼 갖고오는
	}
	void aaa()
	{
		/*this.*/hakbun=1; //this가 생략
		name="홍길동";
		school_name="SIST";
		bbb();
	}
	static void bbb() //static 안에서는 스태틱 만 가능
	{
		school_name="SIST2";
