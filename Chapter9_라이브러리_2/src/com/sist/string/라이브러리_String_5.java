 ...
 *  ------------------------
 *  charAt()
 *  equals() 대소문자 구분 , equalsIgnorecase() : 대소문자구분x
 *  ------- ID,PWD		 -----영어검색 , 영문이름
 *  index of() , lastIndexOf() --- 뒤에서부터 찾는다
 *  -------위치확인 (문자/문자열)
 *  ------ 앞에서부터 찾기
 *  length() : 문자의 갯수
	...
	public static void main(String[] args) {
		...
		for(String s:data2)
		{
			if(s.startsWith("p"))
			{
				...
				{
					Thread.sleep(1000);
				}catch(Exception ex) {} //try ~ catch 나오면 CheckException*/
			}
		}
		...
		// 구분이 되어 있는 문자열이 있는 경우 분리 할떄 split
		...
		String[] color=ss.split("\\?"); // |는 정규식에서 사용하는 기호 \\두개 붙여야함
		// |, ?, ., +, *, ^
		for(String c: color)
		{
		...
		// replace() : 문자/문자열 변경
		// 원형 : String replace(char old,char new) old->new로
