 ...
 *     	  객체=인스턴스 => 동일 개념
 *     -----------------------------------------------------
 *     정적변수		  컴파일러가 컴파일했을떄	  프로그램 종료시까지 유지
 *     (공유변수,		  JVM이 클래스 읽기
 *     클래스변수)		  => 메모리 할당이 자동
 *     
 *     => 메모리할당 없이 사용가능
 ...
 *     	  3) 특수문자 사용 ( _, $)
 *     	  4) 키워드 예약어는 사용 불가 (메소드명, 클래스명은 키워드가 아니다)
 *     	  5) 변수에 공백이 없다
 *     	  -------------------------------- 오라클(컬럼) , 자바스크립트에서도 동일하다
 *     		오라클 (변수=> 컬럼) , 자바스크립트는 자동지정변수
 *     							ES6 => let,const
 *     							let a=10 => a:자동으로 int
 *     							let a=10.5 => a:자동으로 double
 *     							const => final (상수형 변수)
 *     => 약속 사항
 *     	  1) 클래스명은 대문자 시작
 *     		 ------ 인터페이스 포함
 ...
 *       	--------- 제어문 (반복문)
 *       	--------- 오라클은 ++,--는 존재하지 않는다 
 *      => 부정연산자	: ! => boolean으로 표현 true=>false false=>true
 *      			  => 조건문 , 반복문에서 주로 사용
 *      			  => 오라클 : NOT, <>
 ...
 *     		{
 *      %>
 *      <a href="list.jsp?page=<%=curpage>1curpage-1:curpage%>"> 이전</a>
 *      --------------------------------------------------------------- 37~53page => 연산자
 *      
 *       제어문 => 54page
 *       --------------
 ...
 *        			구현부
 *        		}
 *        		접근지정어는 다른 클래스와 통신을 담당 => 공개(public) 메소드는 될수 있으면 public
