...
// 오라클 연결 => 인터페이스 구현
// 인터페이스 => 서로 다른 프로그램을 연결할 때 사용 (리모콘)
...
public class EmpDAO {
	...
	// 오라클 해제 
	  ...
	  *                      ----------------Full Stack
	  */
	 // 기능 : 사용자 요청시에 데ㅣ터베이스 => SQL문장을 만들어서 오라클 연동
	 // 1. 사용자가 사원 전체 목록 요청 
	 public List<EmpVO> empListData()
	 {
		 ...
		 try
		 {
			 ...
			 {
				 EmpVO vo=new EmpVO();
				 vo.setEmpno(rs.getInt(1)); // 데이터형에 맞게 읽어오기
	...
	public EmpVO find(String fd)
	{
		EmpVO vo=new EmpVO();
		 try
		 {
			 ...
			 getConnection();
			 //2.SQL문장 만들기
			 String sql="SELECT * "+"FROM emp "+"Where ename="+"'"+fd+"'";
			 //3. 오라클 전송
			 ps=conn.prepareStatement(sql);
			 //4. 실행후에 결과값 받기
			 ResultSet rs=ps.executeQuery();
	...
	//3-1
	public List<String> find2(String s)
	{
