 ...
 *  method도 메모리에 저장된다
 *  void method() => 이러면 저장 ;찍어서 선언하면 저장x
 *  {
 *  	구현부가 있어야 저장이 된다
 *  }
 *  => 메소드 선언만 된 경우에는 클래스를 저장하지 못한다 (미완성된 클래스)
 *  	(추상클래스, 인터페이스) => 요구사항 분석
 *  			  -------- 데이터베이스 연결 , 윈도우 작업, 네트워크 서버,
 *  						웹구현
 *  	void method(); 추상클래스는 선언만 함 사용자마다 구현하는 내용이 다르기 떄문에
 * ----------------------
 * Stack; LIFO 
 *        지역변수,매개변수 저장되는 위치 => 메모리 자체에서 관리({}이 종료되면 사라진다)
 *        
 *        class A
 *        {
 *        	void method()
 ...
 *        }
 * -----------------------------
 * Heap : 동적메모리 할당(읽을때마다 메모리 할당) => 객체변수 , 배열 (참조변수 값)
 * 		  => 관리를 프로그래머가 한다 => GC(자동메모리 회수)
 * 		  => 객체=null 값을 주면 자동으로 사라진다
 * A.java ==> A.class ===>
 * 		javac 		 java
 ...
 * 	---
 * 	---
 * 	---new 가 붙으면 메모리 할당 한줄씩 읽다가 new가 나오면 메모리할당
 * 	---
 * 	---
 * }
 * -------------------------------------------
 ...
 *  	  사용중
 *  	8-------
 *  	  => 값이 저장 =>10 ==> 메모리 주소에 대한 별칭 a
 *     12-------
 *  배열 같은거 여러개 모으는
 *  	-------------------
 *  		|     |     |
 *  	-------------------
 *  클래스 다른거 여러개 모으고 메소드 들어갈수 있다
 *  	-------------------
 *  	  -------------
 *  	  1
 *        ------------
 ...
 *  		생성되는시점=> new 를 이용해서 메모리 저장공간이 만들어진 경우
 *  		A a=new A()
 *  		  - 인스턴스(객체)
 *  		  a.변수명
 ...
 *  	-------------------------------------------------------------
 *  	--------------------메소드 호출시에 생성-----------------
 *  	  = 지역변수 : 기능처리에 필요한 데이터를 설정 (메소드 종료와 동시에 사라진다)
 ...
 *  		class A
 *  		{
 *  			String addr;  전역변수 메소드 여러군데에서 사용하면 전역변수
 ...
 *  					 다른 클래스와 통신 담당
 *  	   1. 종류
 *  		  인스턴스메소드 : 따로 작동하는 메소드 제어하는 대상이 있는경우
 *  			 String s="Hello Java";
 *  			 s.substring()
 *  			 String s1="Hello Oracle";
 *  			 s1.substring()
 *  		  정적메소드 : 동시에 작동 (static=>자동 저장) 제어하는 대상이 없는경우
 *  			10=>"10" , true="true"
 *  			String.ValueOf(데이터형)
 *  			Math.random()
 ...
 *  	---------------------------
 *  	protected: 같은 패키지안에서만 접근이 가능, 상속이 있는 경우 다른 패키지에 접근이 가능하다
 *  	--------------------------
