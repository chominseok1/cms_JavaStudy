...
public class GenieMusicSystem {
	...
	//인스턴스 블록
	{
		try
		{
			...
			// 구분자 => 모든 태그 구분자 class(중복) / id(중복이 없다)
			// class라 되어있으면 =>. id=> #
			...
			for(int i=0;i<title.size();i++) //저장된 갯수만 큼 갖고오는
			{
				...
				{
					state=s.replaceAll("[0-9]", ""); //숫자전체 를 공백으로 바꿔라
					value=s.replaceAll("[가-힣]", ""); // 한글 전체 를 공백으로 바꿔라
				}
				/*
				 * split
				 * replaceAll
	...
	protected Music[] musicFind(String fd,int type)
	{
		int i=0;
		for(Music m:musics)
		{
		...
		Music[] mm=new Music[i]; //찾은개수 확인 i 그 갯수만큼 배열 생성 배열단점: 고정시켜놓음
		i=0;
		/*
		 *  A a=new A();
		 *  A b=a;
		 */
		for(Music m:musics) //메모리 50개 주소가 있음
		{
			boolean bCheck=false;
			if(type==1)
				{
