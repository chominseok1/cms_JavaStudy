...
/*
 * 		0420 문제3. final 클래스 => 종단 클래스 (java.lang)
 ...
 * 										 버튼 클릭 , 마우스 클릭...
 * 										 게시판 : 글쓰기	
 * 					B d=(B)c; => 명시적 형변환 (강제 형변환)
 * 					-------- 라이브러리에서 많이 사용
 * 					=> 클래스를 저장해서 관리 (클래스) => Collection
 * 					'A' 'BBBBB'
 * 				   1byte 5byte 가변형 => collection
 * 
 * 					'A' 'BBBBBB' ==> 1000byte 배열은 고정적
 * 				class A
 * 				{
 ...
 * 				------------------------------------------
 * 				*** 상속
 * 				1) 호출 ==> 상위클래스의 생성자가 먼저 호출 (기본생성자) 상위 클래스의 생성자에 매개 변수가 있으면 상위 클래스에 기본 생성자 추가
 * 				   class A
 * 					{
 * 					=> public A()
 * 					  public (int a){}
 ...
 * 				B b=new E()
 * 
 * 				B b=(B)new A(); A가가지고 있느 메소드 호출 =>ClassCastException
 * 				------------------------------
 * 				A a=new B();
 * 				B b=(B)a;
 * 		-----------------------------------------------------------------------
 * 		클래스의 종류
 * 		= 추상클래스
 * 		= 인터페이스 => 추상클래스의 단저을 보완 (클래스와 동일하게 취급)
 * 		** 클래스(단일) / 인터페이스 (다중)
 ...
 * 					  }
 * 					  구현이 안된 메소드(선언)
 * 						[접근지정어] abstract 리턴형 메소드명(매개변수..); abstract ;
 * 								  -------                     --
 * 					--------------------------
 * 					생성자 , 오버로딩이 가능
 * 					--------------------------
...
class AAA
{
	public void aaa() {}
	public AAA() {}
	int a=10; // 설계시 순서는 마음대로 일반적으로 변수 생성자 메소드 순서
...
// new A() 디폴트 생성자가 먼저 호출 되어야 되기 떄문에 오류 매개변수의 값을 못넣음
class B extends A
{
	int b=20;
	public B()
	{
		//super();// 상위 클래스의 메모리할당이 필요함 자동으로 상위클래스의 생성자를 먼저 호출
	}
}
//A a=new B() => A에 있는 데이터만 사용, 메소드는 변경된 것만 사용
// 하위클래스에서 추가된 메소드, 변수는 사용할 수 없다
// B b=new B()
public class 정리_7장 { //String,Math,System final 클래스 여서 상속 x
	...
	public static void main(String[] args) {
		...
		list.add(1);
		list.add(new 정리_7장());
		String s=(String)list.get(0); //object여서 String으로 형변환
		
		//B b=(B)new A(); //에러 형변환 잘못됬다
		/*A a=new B();
		B b=(B)a; 
		
		String s1="Hello";
		String s2="Hello";
		String s3=new String("Hello"); //new를 주면 새로운 주소 생성
		if(s1==s2)
