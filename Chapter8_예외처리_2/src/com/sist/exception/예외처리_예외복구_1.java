 ...
 * 			  -----
 * 			
 * 			Object : 모든 클래스의 상위클래스(모든 클래스는 Object 상속을 받는다)
 * 			  |
 ...
 *			  (java.io)
 *			=>SQLException : 오라클 연결 (오라클이 인식하는 언어 : 오라클은 자바가 아니라 sql)
 *			  JDBC ==> DBCP ==> ORM (MyBatis,JPA)
 *			  Web = 핵심(데이터베이스 연동)
 *			  (java.sql)
 *			=> MalformedURLException : IP/Web URL
 *			   (java.net)
 *			=>ClassNotFoundException : 클래스 이름으로 메모리 할당 => 클래스명, 패키지 다른경우 오류)
 *			  = 스프링에서 주로 사용하는 기술 (리플렉션)
 *									----------- 단점 private 까지 접근이 가능
 *
 * 			A a=new A(); Class.forName("A")
 * 			
 * 			=>InterruptedException : 쓰레드 (한개의 프로세스 안에서 여러개의 프로그램을 동시에 사용)
 * 			  충돌과 관련
 ...
 * 							  int  => Integer
 * 							  double => Double
 * 					Integer.parseInt(" 10") 오류발생 공백x =>윈도우/Web(문자열만 사용)
 * 					 웹에서 전송된 데이터를 받는 경우
 * 					 ----------------------
 ...
 *          	A a; , int[] arr; ==> null
 *          	--- A a=new A();
 *          ClasCastException : 클래스 형변환이 잘못된 경우
 *          class A
 *          class B extends A
 ...
 *     		try
 *     		{
 *     				정상수행이 가능한 코딩 => 지금까지 소스가 들어가는 부분
 *     		}catch (예외처리 종류 설정)
 *     		{	
 *     			수행과정에서 오류가 났을 때 처리되는 부분
 *     		}
 *     		-----------------------------------------------------
 *     
